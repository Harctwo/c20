...
| `hl` | **Lightmap Half-Life**. Scales the distance light travels from the light source to nearby surfaces.
...
3. In the first example we are looking for a shader tag named metal_roof from the city collection. Any surfaces using this material would have light travel twice it's regular distance making light rays longer.
